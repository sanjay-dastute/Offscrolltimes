...
function Quiz() {
  ...
  return <>
    ...
    <div className="play-card-body">
      {phase === 'ready' && <><div className="play-badge" aria-hidden="true">3</div><h3>Ready, offscrollers?</h3><p>Three questions. Thirty seconds. Can you spot the little traps?</p><label className="play-check"><input type="checkbox" checked={!timed} onChange={e => setTimed(!e.target.checked)} /> Play without a timer</label><button type="button" className="play-primary" onClick={start}>Let’s play <span aria-hidden="true">→</span></button></>}
